...
def add_another_expense():
  ...
  if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get():
    mb.showerror('Fields empty!', "Please fill all the missing fields before pressing the add button!")
     ...
     connector.execute('INSERT INTO ExpenseTracker (Date, Payee, Description, Amount, ModeOfPayment) VALUES (?, ?, ?, ?, ?)',(date.get_date(), payee.get(), desc.get(), amnt.get(), MoP.get()))
     connector.commit()
...
def edit_expense():
  ...
  def edit_existing_expense():
     ...
     connector.execute('UPDATE ExpenseTracker SET Date = ?, Payee = ?, Description = ?, Amount = ?, ModeOfPayment = ? WHERE ID = ?',(date.get_date(), payee.get(), desc.get(), amnt.get(), MoP.get(), contents[0]))
     connector.commit()
...
# Buttons' Frame
Button(buttons_frame, command=remove_expense, text='Delete Expense', font=btn_font, width=25, bg=hlb_btn_bg).place(x=30, y=5)
